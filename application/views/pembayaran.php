          ...
          <div class="order-col">
            ...
            <div>
              <strong class="order-total">
                Rp. <?php echo $datatop->nominal; ?>
              </strong>
            </div>
          </div>
        </div>
        <label>Upload Bukti:</label><br>
        <form method="post" name="form_uploadproduk" action="<?php echo base_url('Pembayaran/prosesPembayaran'); ?>" enctype="multipart/form-data">
        <button class="primary-btn" disabled>
          <input type="file" name="gambar" required oninvalid="this.setCustomValidity('gambar bukti tidak boleh kosong dan merupakan jenis file jpg, jpeg, & png')" oninput="setCustomValidity('')">
        </button>
        <input type="hidden" name="id" value="<?php echo $datatop->id_topup; ?>">
        <input type="hidden" name="status" value="menunggu">
        <button class="primary-btn order-submit-form"><i class="fa fa-upload"></i> Upload Bukti</button>
        </form>
      </div>
      <!-- /Order Details -->
      <?php
      }elseif ($datatop->status_topup=='menunggu') {
      ?>
      <!-- Order Details -->
      <div class="col-md-5 order-details">
        <div class="section-title text-center">
